...
const execute = async () => {
    try {
        ...
        // We are gonna check if testing is set up and only and only then push the coverage file
        ...
        if (config.test && Object.keys(config.test).length !== 0) {
            filesToCompress.push(path.join(cwd, constants.fs.directory, constants.fs.coverage));
        }
