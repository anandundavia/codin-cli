...
const execute = async () => {
    try {
        ...
        const json = path.join(cwd, constants.fs.directory, constants.fs.lint);
        const summary = path.join(cwd, constants.fs.directory, constants.fs.summary);
        const quality = path.join(cwd, constants.fs.directory, constants.fs.quality);
        const compressedFile = await compress.execute([json, summary, quality]);
