...
const authSotre = require('../services/authstore.service');
...
const execute = () => new Promise(async (resolve, reject) => {
    const credentials = await authSotre.getCredentials();
    ...
    if (credentials) {
        ...
        return server.login(credentials.email, credentials.password)
            ...
            .catch((err) => {
                ...
                const credentialsPath = path.join(os.homedir(), fs.creadentials);
                logger.error('Failed to login using saved credentials.');
                logger.info(`\nYou might want to remove the '${credentialsPath}' file. And run 'codin -i' again.\n`);
                reject(err);
            });
    }
    ...
    return server.login(login.email, login.password).then((data) => {
        ...
        authSotre.setCredentials(login);
        resolve(data);
    }).catch(reject);
});
