...
const createRuleFile = cwd => new Promise((resolve) => {
    const writePath = path.join(cwd, constants.fs.directory, constants.fs.tslintRules);
...
const removeRuleFile = cwd => new Promise((resolve) => {
    const filePath = path.join(cwd, constants.fs.directory, constants.fs.tslintRules);
...
const execute = cwd => new Promise(async (resolve, reject) => {
    try {
        ...
        if (isFileCreated) {
            const configPath = path.join(constants.fs.directory, constants.fs.tslintRules);
        ...
        const outputPath = path.join(constants.fs.directory, constants.fs.lint);
