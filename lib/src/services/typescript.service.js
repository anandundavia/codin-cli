...
const TypeScript = function TypeScript() {
     ...
     */
    this.compile = cwd => new Promise(async (resolve, reject) => {
        try {
            ...
            logger.info('Typescipt successfully compiled');
     ...
     * @param {string} cwd Path to the current project root
     */
    this.cleanup = cwd => new Promise(async (resolve, reject) => {
        try {
            const tslintConfigs = JSON.parse(fs.readFileSync('tsconfig.json', 'utf-8'));
            const output = path.join(cwd, tslintConfigs.compilerOptions.outDir);
            // TODO: if the output dir is /dist/tsc-out then only contents of tsc-out
            // will get deleted.
            // Findout how to delete the <cwd>/dist folder
            // You may want to use path library
